...
const FeedbackPage = () => {
  ...
  const [formOpen, setFormOpen] = useState(false); // ✅ NEW
  ...
  const handleFeedbackSubmit = (data: FeedbackFormData) => {
    ...
    // Optional: call POST /feedback API
    toast({
  ...
  const getAverageRating = (f: Feedback) => {
    const ratings = [f.driverRating, f.vehicleRating, f.serviceRating].filter(Boolean);
  ...
  if (loading) return <div className="flex justify-center items-center h-64">Loading feedback...</div>;
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button onClick={() => setFormOpen(true)}>➕ Add Feedback</Button> {/* ✅ NEW */}
      </div>
          ...
          <Card key={item.id} className="hover:shadow-md transition-shadow">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                ...
                <Badge variant="outline">Avg: {getAverageRating(item)}★</Badge>
              </div>
            </CardHeader>
            <CardContent>
              {item.trip?.booking && (
              ...
              <div className="mt-4 flex space-x-2">
                ...
                <Button size="sm" variant="outline">View Trip</Button>
              </div>
            </CardContent>
          </Card>
        ))}
      ...
      <FeedbackForm
        ...
        tripInfo={{
          from: 'Kukatpally',
          to: 'Adambakkam',
          driverName: 'Kiran Kumar Sabhapathi',
        }}
      />
    </div>
  );
