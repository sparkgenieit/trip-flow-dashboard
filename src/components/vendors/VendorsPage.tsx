...
interface Vendor {
  ...
  name: string;
  companyReg: string;
  vendor?: { email?: string; phone?: string };
}
...
const VendorsPage = () => {
  ...
  const filteredVendors = (vendors || []).filter(
    (v) =>
      ...
      v.companyReg?.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button asChild>
          <Link to="/dashboard/vendors/new/tabs">
            ...
            {filteredVendors.map((vendor) => (
              <Card key={vendor.id} className="shadow-sm border rounded-lg p-4 space-y-3">
                ...
                <div className="text-sm space-y-1">
                  <p>
                    <span className="font-medium">Company Reg:</span> {vendor.companyReg}
                  </p>
                </div>

                <div className="flex gap-2">
                  {/* EDIT now navigates to /dashboard/vendors/:id/tabs */}
                  <Button size="sm" variant="outline" asChild>
                    <Link to={`/dashboard/vendors/${vendor.id}/tabs`}>Edit</Link>
                  </Button>
                  ...
                  <Button
                    size="sm"
                    variant="destructive"
                    onClick={() => handleDelete(String(vendor.id))}
                  >
                    Delete
                  </Button>
                </div>
